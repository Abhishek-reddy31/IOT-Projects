...
void loop() {
  ...
  if (distanceFront < 20 || distanceLeft < 20 || distanceRight < 20) {
    ...
    // Avoid continuous turns by introducing a delay and checking sensors again
    delay(1000);
...
bool checkObstacles() {
  readUltrasonicSensors();
  if (distanceFront < 20 || distanceLeft < 20 || distanceRight < 20) {
  //if (distanceFront < 20 || distanceRight < 20) {
    stopMotors();
...
void moveBackwardUntilClear() {
  ...
  //while (distanceFront < 20 && distanceRight < 20) {
...
void turnLeft() {
  ...
  digitalWrite(in3, HIGH);
  digitalWrite(in4, LOW);
  analogWrite(enA, 50);  // Adjust as needed
  analogWrite(enB, 150);  // Adjust as needed
}
...
void turnRight() {
  ...
  digitalWrite(in3, LOW);
  digitalWrite(in4, LOW);
  analogWrite(enA, 150);  // Adjust as needed
  analogWrite(enB, 50);  // Adjust as needed
}
